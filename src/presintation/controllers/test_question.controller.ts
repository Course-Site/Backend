import {
  ...
  import { ITestQuestionService } from 'src/use-cases/test_question/interface/service/test_question.service.interface';
  import {
	ApiBearerAuth,
	ApiTags,
	ApiOperation,
...
import { CreateTestQuestionDto } from '../dto/test/create.test_question.dto'
  ...
  export class TestQuestionController {
	...
	@ApiResponse({ status: 200, description: 'test question data has been changed.' })
	@ApiResponse({ status: 404, description: 'Test question not found.' })
	async update(@Param('id') id: string, @Body() testQuestion: ITestQuestionEntity) {
