...
export class TestEvaluationService {
  constructor(
    ...
    @Inject('testResultRepository') private testResultRepository: ITestResultRepository,
  ) {}

  async evaluateTestSubmission(userId: string, testId: string, answers: IEvaluationDto[]) {
  ...
  evaluateSingle(correctIds: string[], selectedIds: string[], questionScore: number): number {
